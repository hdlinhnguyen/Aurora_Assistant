...
// Đơn giá ước tính (USD trên 1 triệu token) — xấp xỉ mức phổ biến của model 70B-instruct;
// chỉnh qua các hằng số này nếu đổi model/đơn giá.
const (
	pricePerMInputUSD  = 0.30
	pricePerMOutputUSD = 0.60
	usdToVND           = 25000.0
	monthlyQuotaUSD    = 50.0 // ngưỡng quota tháng để tính % còn lại + circuit breaker
)

// Model lấy theo cấu hình runtime (OPENAI_MODEL) để dashboard hiển thị đúng model đang chạy.
