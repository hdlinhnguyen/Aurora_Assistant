...
func main() {
	...
	if adminCount == 0 {
		...
		_, err := authSvc.Register("admin@aurora.edu.vn", "demo123", "Quản trị viên Hệ thống", "admin")
		if err != nil {
			log.Printf("Failed to create default admin: %v", err)
		}
	}

	// Seed Demo Accounts & Mock Statistics natively if not in production or SEED_DEMO is true
	if os.Getenv("APP_ENV") != "production" || os.Getenv("SEED_DEMO") == "true" {
		log.Println("Seeding demo accounts and mock data...")
		...
		// Kích hoạt giáo viên Demo
		config.DB.Model(&model.User{}).Where("email = ?", "teacher@aurora.edu.vn").Update("status", "active")

		// Tạo lớp học Demo và phân bổ học sinh
		var teacherID string
		config.DB.Table("users").Where("email = ?", "teacher@aurora.edu.vn").Select("id").Row().Scan(&teacherID)
		
		config.DB.Exec("DELETE FROM classrooms WHERE teacher_id = ?", teacherID)
		
		classIdVal := uuid.New()
		config.DB.Create(&model.Classroom{
			ID:        classIdVal,
			Name:      "Lớp 7A1 (Demo)",
		...
		config.DB.Model(&model.User{}).Where("email IN (?, ?, ?)", "studentA@aurora.edu.vn", "studentB@aurora.edu.vn", "studentC@aurora.edu.vn").Update("classroom_id", classIdVal)
